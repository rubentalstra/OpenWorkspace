...
**Implementation note (UI foundation, as built — supersedes the rust-ui specifics in §5/Appendix E above):** the `ui` crate ships as a stable, first-party Leptos kit, not vendored rust-ui verbatim. rust-ui's `icons`/`leptos_ui` force `leptos/nightly` and are **not** used; styling is `tw_merge` behind a first-party `cn!` facade plus our own `clx!`/`void!`/`variants!` macros (under `tw/`), and icons are `leptos_icons` 0.7.1 + `icondata` 0.7 (Lucide), used directly. **All interactivity is pure Leptos — zero JavaScript:** the rust-ui inline-`<script>` components and `.js` assets were rewritten to `RwSignal`+`<Show>`+`window_event_listener` (and deleted). Tailwind v4 builds via cargo-leptos's bundled binary (**Pipeline A**: `[[workspace.metadata.leptos]] tailwind-input-file = apps/web/style/tailwind.css` with `@source` globs), not an `npx @tailwindcss/cli --watch`; `crates/ui` carries no `[package.metadata.leptos]`. Shipped: 83 components + 18 hooks (clippy `-D warnings` clean on ssr + hydrate, fmt + leptosfmt), pure-Leptos dark mode (`ThemeMode::init()`), and an axe-core-gated `/ui` showcase. Still pending: rust-ui **charts** (CDN-JS, CSP review — later phase) and `AutoForm`'s `#[derive(AutoForm)]` proc-macro (consumer-side tooling).

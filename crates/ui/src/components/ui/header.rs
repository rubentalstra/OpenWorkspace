...
pub fn NavMenuTrigger(children: Children, #[prop(into, optional)] class: String) -> impl IntoView {
    ...
    view! {
        ...
        >
            <span>{children()}</span>
            <Icon icon=icondata::LuChevronDown attr:class="relative ml-1.5 opacity-75 transition duration-300 lucide lucide-chevron-down top-[1px] size-3 group-hover/dropdown:rotate-180 group-hover/dropdown:translate-y-px group-focus-within/dropdown:rotate-180 group-focus-within/dropdown:translate-y-px" />
        </a>
    }
}
...
/// mirrors the active value onto the `dark` class of the document root so
/// Tailwind's dark variant takes effect.
#[derive(Debug, Clone, Copy)]
pub struct ThemeMode {
    state: RwSignal<bool>,
}

/// Returns the [`ThemeMode`] provided by [`ThemeMode::init`].
///
/// Requires that [`ThemeMode::init`] ran earlier in the render tree; absence is
/// a setup invariant rather than a runtime condition.
#[must_use]
...
impl ThemeMode {
    ...
    /// document root's `dark` class in sync.
    ///
    /// Returns the same [`ThemeMode`] handle that is placed in context.
    #[must_use]
    pub fn init() -> Self {
        let theme_mode = Self {
    ...
    /// Sets dark mode to `dark` (`true` = dark, `false` = light) and persists it.
    ...
    /// Returns `true` when dark mode is active, tracking reactively.
    #[must_use]
    pub fn is_dark(&self) -> bool {
        self.state.get()
    }

    /// Returns `true` when light mode is active, tracking reactively.
    #[must_use]
    ...
    fn resolve_initial_preference(self) {
        ...
        Effect::new(move |_| {
            ...
            // this scope; it lives for the page's lifetime to track the OS scheme.
            on_change.forget();
        });
    }
